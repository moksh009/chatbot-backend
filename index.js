...
async function getAvailableBookingDays() {
  try {
    ...
    const days = [];
    const now = new Date(new Date().toLocaleString('en-US', { timeZone: 'Africa/ahmedabad' }));
...
function getPaginatedServices(page = 0) {
  const servicesPerPage = 8; // Show 8 services + "Ask Coach" + "Choose Another Service"
...
const codeClinicDoctors = [
  { id: 'doctor_steven', title: 'Turf 1' },
  { id: 'doctor_angella', title: 'Turf 2' }
];

// Helper: get pricing info from knowledge base
const codeClinicPricing = [
  { service: 'Cricket', price: '1200/hour' },
  { service: 'Football', price: '1000/hour' },
  { service: 'Basketball', price: '4500/hour' },
  { service: 'Hockey', price: '1500/hour' },
  { service: 'Volleyball', price: '1300/hour' },
  { service: 'Pickleball', price: '4500/hour' },
  { service: 'Tennis', price: '1500/hour' },
  { service: 'Badminton', price: '1500/hour' }
];
...
async function handleUserChatbotFlow({ from, phoneNumberId, messages, res }) {
  ...
  if (userMsgType === 'text' && userMsg && GREETING_WORDS.some(w => userMsg.trim().toLowerCase().startsWith(w))) {
    await sendWhatsAppList({
      ...
      rows: [
        ...
        { id: 'user_reschedule_appt', title: 'Reschedule Booking 🔁' },
  ...
  if (userMsgType === 'text' && (!session.step || session.step === 'home' || session.step === 'home_waiting' || session.step === 'faq_menu' || session.step === 'appt_day' || session.step === 'appt_pick_day_waiting' || session.step === 'appt_time_waiting' || session.step === 'ask_question_topic' || session.step === 'faq_await')) {
    ...
    const bookingKeywords = ['book appointment', 'make appointment', 'schedule appointment', 'book visit', 'see coach', 'book turf session'];
    ...
    const prompt = `You are moksh, a friendly turf booking assistant for TURF BOOKING in ahmedabad, Uganda.
...
4. If asked about software/technology: "We use modern booking software for field management and scheduling."
5. If asked about pricing: Mention 2-3 top services only
6. If asked about hours: "We're open Monday-Sunday, 6 AM to 10 PM"
7. If question is NOT about turf booking services: Politely redirect to turf booking topics
...
Provide a SHORT, PRECISE response:`;
    ...
    try {
      const completion = await openai.chat.completions.create({
        ...
        messages: [
          { 
            role: 'system', 
            content: 'You are moksh, a friendly turf booking assistant for Turf Booking in ahmedabad. Be conversational, warm, and helpful. Use natural language, appropriate emojis, and always sound like a real person. Reference the knowledge base for accurate information.' 
          },
  ...
  if (session.step === 'home_waiting') {
    if (userMsg === 'user_schedule_appt') {
      ...
      await sendWhatsAppList({
        ...
        to: from,
        header: 'Book Turf ⚽',
        body: 'Which sports do you want to play?',
        button: 'Select Sports',
        rows: paginatedServices.services
      });
      session.step = 'choose_service';
      session.data.servicePage = 0;
      ...
      codeClinicPricing.forEach(item => { // Turf pricing, see array for details
  ...
  if (session.step === 'choose_service') {
    ...
    if (userMsg === 'service_ask_doctor') {
      await sendWhatsAppText({
        phoneNumberId,
        to: from,
        body: 'Great choice! 😊 I\'ll connect you with our coach for a personalized turf session. Please provide your name and we\'ll schedule a session for you.'
      });
      session.data.chosenService = 'Coach Turf Session';
    ...
    if (chosen) {
      ...
      await sendSmartButtonsOrList({
        phoneNumberId,
        to: from,
        header: `Great! in which turf would you like to play?`,
        body: 'Choose your turf:',
  ...
  if (session.step === 'choose_doctor') {
    ...
    if (chosen) {
      ...
      await sendWhatsAppList({
        phoneNumberId,
        to: from,
        header: `Choose a date for your playing`,
      ...
      await sendSmartButtonsOrList({
        phoneNumberId,
        to: from,
        header: `In which turf would you like to play?`,
        body: 'Choose your turf:',
        buttons: codeClinicDoctors
      });
      session.step = 'choose_doctor';
    ...
    if (date || session.data.date) {
      ...
      try {
        ...
        if (!slotResult.slots || slotResult.slots.length === 0) {
          ...
          const nowEAT = new Date().toLocaleString('en-US', { timeZone: 'Africa/ahmedabad' });
      ...
      try {
        ...
        if (previousAppointments.length > 0) {
          ...
          if (hasConsentHistory) {
            ...
            let consentStatus = '';
            let confirmationBody = `✅ *Booking Summary*\n\n👤 *Name:* ${session.data.name}\n📅 *Date:* ${session.data.date}\n🕒 *Time:* ${session.data.time}\n👨‍🏫 *Coach:* ${session.data.doctor || 'Not specified'}\n🏟️ *Service:* ${session.data.chosenService || 'General Turf Session'}\n\n📱 *Phone:* ${session.data.phone}\n\n🔔 *Communication Preferences:*\n`;
        ...
        await sendWhatsAppButtons({
          ...
          to: from,
          header: '📋 Booking Summary',
          body: `*Booking Details:*\n\n👤 *Name:* ${session.data.name}\n📅 *Date:* ${session.data.date}\n🕒 *Time:* ${session.data.time}\n👨‍🏫 *Turf:* ${session.data.doctor || 'Not specified'}\n🏟️ *Service:* ${session.data.chosenService || 'General Turf Session'}\n\n📱 *Phone:* ${session.data.phone}\n\n🔔 *Communication Preferences:*\nWe'd like to send you:\n• Booking reminders\n• Birthday wishes\n\nPlease choose your preference:`,
          buttons: [
            { id: 'consent_confirm_all', title: '✅ Accept All' },
            { id: 'consent_reminders_only', title: '📅 Reminders Only' },
            { id: 'consent_none', title: '❌ No Thanks' }
  ...
  if (session.step === 'appt_confirm_with_previous_consent') {
    if (userMsg === 'confirm_with_previous_consent') {
      ...
      let confirmationBody = `✅ *Booking Confirmed*\n\n📅 *Date:* ${session.data.date}\n🕒 *Time:* ${session.data.time}\n👨‍🏫 *Coach:* ${session.data.doctor || 'Not specified'}\n\n📍 *Location:* Turf Location\n🗺️ *Map:* https://maps.google.com/?q=Turf+Location\n\n⏰ *Please arrive 15 minutes early* for your booking.`;
      ...
      try {
        ...
        if (allAppointments.length === 1) {
          ...
          await sendWhatsAppButtons({
            ...
            body: `Found your booking:\n${foundAppt.summary}\nDate: ${foundAppt.date}\nTime: ${foundAppt.time}\nDo you want to ${session.step === 'cancel_lookup' ? 'cancel' : 'reschedule'} this booking?`,
            buttons: [
...
app.post('/', async (req, res) => {
  ...
  try {
    ...
    if (
      ...
      console.log('User logic (TURF BOOKING flow) triggered for', from);
