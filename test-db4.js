...
async function fixData() {
    ...
    for (const order of orders) {
        if (lead.activityLog && lead.activityLog.some(log => log.details && log.details.includes(order.orderId))) {
            continue;
        }
        ...
        const itemNames = order.items && order.items.length > 0 ? order.items.map(i => `${i.quantity}x ${i.name}`).join(', ') : 'Items';
    ...
    if (newActivity.length > 0) {
        ...
        if (!lead.addToCartCount) {
            updateObj.$inc = { addToCartCount: 1, checkoutInitiatedCount: 1 };
        }

        await AdLead.findByIdAndUpdate(lead._id, updateObj, { new: true, runValidators: false });
