...
async function detectLanguage(text, apiKey) {
  ...
  try {
    ...
    const langCodeRaw = await generateTextFast(prompt, apiKey);
...
async function translateToUserLanguage(text, langCode, apiKey) {
  ...
  try {
    ...
    const translated = await generateTextFast(prompt, apiKey, { timeout: 6000 }); // Slightly more for translation
 ...
 */
async function normalizeIntent(text, apiKey) {
  ...
  try {
    ...
    const intentRaw = await generateTextFast(prompt, apiKey);
