...
const log = require('../core/logger')('WalletService');

const LOYALTY_EXPIRY_DEFAULT_DAYS = 90;

function computeLoyaltyExpiresAt(loyaltyConfig = {}) {
    const days = Number(loyaltyConfig.expiryDays);
    const effectiveDays = Number.isFinite(days) && days > 0 ? days : LOYALTY_EXPIRY_DEFAULT_DAYS;
    return new Date(Date.now() + effectiveDays * 24 * 60 * 60 * 1000);
}

async function syncAdLeadLoyaltyFields({ clientId, phone, balance, tier, loyaltyConfig }) {
    const cleanPhone = normalizePhone(phone);
    const loyaltyExpiresAt = computeLoyaltyExpiresAt(loyaltyConfig);
    await AdLead.findOneAndUpdate(
        { phoneNumber: cleanPhone, clientId },
        { $set: { loyaltyPoints: balance, loyaltyTier: tier, loyaltyExpiresAt } },
        { upsert: false }
    );
}
...
async function processOrderForLoyalty(clientId, phone, orderAmount, orderId) {
    try {
        ...
        const { isLoyaltyEnabled } = require('../core/featureFlags');
        ...
        await syncAdLeadLoyaltyFields({
            ...
            log.warn(`AdLead loyalty sync failed for ${cleanPhone}: ${err.message}`);
        });
...
async function addPoints(clientId, phone, points, reason = 'Flow reward') {
    ...
    await syncAdLeadLoyaltyFields({
        ...
        balance: wallet.balance,
        tier: wallet.tier,
        loyaltyConfig: client?.loyaltyConfig || {},
    }).catch(() => {});
...
module.exports = {
    computeLoyaltyExpiresAt,
    syncAdLeadLoyaltyFields,
