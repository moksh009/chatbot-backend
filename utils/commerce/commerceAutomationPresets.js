...
/**
 * Retired order message rule IDs — do not re-add.
 * These were removed because third-party logistics partners (Shiprocket, Delhivery, etc.)
 * already send these status updates natively when connected, making TopEdge duplicates
 * confusing and costly (double WA conversation fee for zero incremental value).
 * See TOPEDGE-SYSTEM-REFERENCE.md Part I for full rationale.
 *
 * Coverage: sys_order_* prefix, sys_financial_* prefix, retired sys_fulfillment_*,
 * bare legacy slots (pending/paid/shipped/delivered/cancelled/cod), and
 * status_* legacy migration IDs.
 */
const RETIRED_ORDER_MESSAGE_RULE_IDS = new Set([
  // sys_order_* — legacy order-notification system rules
  'sys_order_pending',
  'sys_order_paid',
  'sys_order_shipped',
  'sys_order_delivered',
  'sys_order_cancelled',
  // sys_financial_* — payment-status rules retired Jun 2026
  'sys_financial_paid',
  'sys_financial_pending',
  'sys_financial_refunded',
  'sys_financial_partially_paid',
  'sys_financial_voided',
  // sys_fulfillment_* except sys_fulfillment_unfulfilled (still canonical as "Order placed")
  'sys_fulfillment_fulfilled',
  'sys_fulfillment_partial',
  // status_* — generated by normalizeStatusMappings legacy migration
  'status_pending',
  'status_paid',
  'status_shipped',
  'status_delivered',
