...
async function awardLoyaltyPoints({ clientId, phone, orderId, orderAmount, isBackfill = false }) {
  try {
    ...
    const { syncAdLeadLoyaltyFields } = require('./walletService');
    await syncAdLeadLoyaltyFields({
      ...
      console.error('[LoyaltyEngine] AdLead loyalty sync failed:', err.message);
    });
...
async function sendReminder({ clientId, phone }) {
    ...
    const { sendWhatsAppTemplate } = require('../meta/whatsapp');
