 ...
 * Automatic Shopify assignment runs on orders/fulfilled via warrantyEngine only.
 * This function remains for public registration and manual/API flows.
 */
  ...
  try {
    ...
    for (const item of lineItems) {
      ...
      const warrantyRecord = await WarrantyRecord.create({
        ...
        shopifyOrderId,
        shopify_internal_id: String(orderData.id || shopifyOrderId),
        shopify_order_name: String(orderData.name || shopifyOrderId),
        productId,
  ...
  if (client.brand?.warrantyWhatsappEnabled) {
    try {
      ...
      const result = await sendForAutomation({
        clientId: client.clientId,
        phone,
        slotId: 'wizard_warranty',
        metaName: 'warranty_confirmation',
        contextType: 'warranty',
      ...
      if (result?.whatsapp?.sent) {
        ...
        log.warn(
          `WhatsApp warranty skipped for ${phone}: ${result?.failureCode || result?.whatsapp?.reason}`
        );
      }
    } catch (err) {
...
async function manualRegister(client, phoneNumber, data) {
  try {
    ...
    const warrantyRecord = await WarrantyRecord.create({
      ...
      shopify_internal_id: String(shopifyOrderId),
      shopify_order_name: String(shopifyOrderId),
