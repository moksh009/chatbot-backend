...
async function updateLeadWithScoring(phoneNumber, clientId, incrementFields = {}, stringUpdates = {}, extraData = {}) {
  try {
    ...
    if (updatedLead && updatedLead.leadScore >= 70) {
        const { logPulse } = require('../core/activityLogger');
...
/**
 * Recomputes scores for all leads — delegates to canonical waterfall engine (D3).
 */
async function recomputeAllScores(clientId) {
  const { recomputeAllWaterfallScores } = require('../core/scoringHelper');
  return recomputeAllWaterfallScores(clientId);
}
