...
async function sendInteractive(client, phone, bodyText, buttons, envelopeOpts = {}) {
  const interactive = {
    type: 'button',
  ...
  const {
    ...
    idempotencyCod,
  } = require('../messaging/cronEnvelopeSend');

  if (!envelopeOpts.orderId) {
    throw new Error('orderId_required_for_cod_envelope');
  }
  ...
  const out = await cronEnvelopeSend({
    ...
    phone,
    contactId,
    idempotencyKey: idempotencyCod({
      orderId: envelopeOpts.orderId,
      stage: envelopeOpts.stage || 'confirm',
      contactId,
      phone,
    }),
    payload: { interactive, text: bodyText },
    context: {
      source: envelopeOpts.source || 'rtoProtectionService',
      orderId: String(envelopeOpts.orderId),
    },
  });
  if (!out.useLegacy) {
    const outcome = handleCronEnvelopeOutcome(out);
    if (outcome === 'sent' || outcome === 'duplicate') return;
    throw new Error(out.reason || 'envelope_blocked');
  }
  throw new Error(out.reason || 'no_contact_for_envelope');
}
...
async function maybeSendCodConfirmationAfterOrderCreate(client, orderDoc) {
  ...
  try {
    await sendInteractive(
      ...
      {
        orderId: orderDoc.orderId || orderDoc.shopifyOrderId || oid,
        stage: 'confirm',
        source: 'rtoProtectionService:cod_confirmation',
      }
    );
...
async function maybeSendNdrRescueFromFulfillment(client, fulfillment, io) {
  ...
  const tpl = cfg.ndrTemplateName || 'rto_ndr_rescue';

  try {
    const AdLead = require('../../models/AdLead');
    const lead = await AdLead.findOne({ clientId: client.clientId, phoneNumber: clean })
      .select('_id')
      .lean();
    const {
      cronEnvelopeSend,
    ...
    const envOut = await cronEnvelopeSend({
      client,
      clientId: client.clientId,
      intent: 'utility',
      phone: clean,
      ...
      payload: {
        templateName: tpl,
        templateLanguage: cfg.ndrTemplateLanguage || 'en',
        components: [],
      },
      context: { source: 'rtoProtectionService:ndr_rescue', orderId: shopifyRef },
    });

    if (!envOut.useLegacy) {
      const outcome = handleCronEnvelopeOutcome(envOut);
      ...
      const result = await sendForAutomation({
        ...
        contextData: {
          ...
          extra: { shopify_order_id: shopifyRef },
        },
      });
      if (!result?.whatsapp?.sent) {
        throw new Error(result?.failureCode || result?.whatsapp?.reason || 'ndr_not_sent');
      }
    }
