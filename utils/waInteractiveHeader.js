...
function buildInteractiveHeaderFromNodeData(data) {
  ...
  if (!ht) {
    if (img) return { type: "image", image: { link: clip(img, 2048) } };
    if (vid) return { type: "video", video: { link: clip(vid, 2048) } };
    if (doc) return { type: "document", document: { link: clip(doc, 2048), filename: docName } };
