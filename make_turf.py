...
turf_code = """const express = require('express');
...
const codeClinicServices = [
  { id: 'service_football', title: 'Football Booking (5v5 - 8v8)' },
  { id: 'service_cricket', title: 'Box Cricket' },
  { id: 'service_pickleball', title: 'Pickleball Court' },
  { id: 'service_volleyball', title: 'Volleyball Court' }
];
...
async function handleUserChatbotFlow({ from, phoneNumberId, messages, res, clientConfig, io }) {
  ...
  if (session.step === 'appt_day') {
      ...
      if (selectedDay) {
        ...
        session.data.date = selectedDay.id; 
...
with open("routes/clientcodes/turf.js", "w") as f:
    f.write(turf_code)

print("Updated turf.js successfully.")
