    ...
    try {
      ...
      for (const campaign of pendingEvaluations) {
        try {
          ...
          for (let i = 0; i < holdbackMessages.length; i += BATCH_SIZE) {
            const batch = holdbackMessages.slice(i, i + BATCH_SIZE);
            ...
            if (i + BATCH_SIZE < holdbackMessages.length) await sleep(1000);
          }
          ...
          for (let i = 0; i < phones.length; i += BATCH_SIZE) {
            ...
            await Promise.allSettled(batch.map(async (row) => {
              ...
              try {
                ...
                // If still empty, try to auto-inject from synced template
            ...
            if (i + BATCH_SIZE < phones.length) await sleep(1000);
          }
